...
2. Configurá la cadena de conexión `GestionLibreriaDb` en `appsettings.json` (SQL Server).
3. Aplicá las migraciones de Entity Framework:
   ```bash
   dotnet ef database update --project GestionLibreria.Infrastructure --startup-project GestionLibreria.Presentation
   ```
4. Ejecutá la API:
   ```bash
   dotnet run --project GestionLibreria.Presentation
   ```
5. Accedé a la documentación OpenAPI en `/openapi` (entorno de desarrollo).

## 📌 Estado del proyecto

Funcional: CRUD completo de las cuatro entidades principales con validaciones de negocio en el dominio. Próximos pasos sugeridos: autenticación/autorización, tests unitarios sobre la capa de dominio, y paginación en los endpoints de listado.
